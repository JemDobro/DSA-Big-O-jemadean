...
// Linear time O(n) -- the larger the num, the more time it will take
...
// Linear time O(n) -- basic arithmetic performed on each item in the array one by one.  If the array size increases, the time needed will increase proportionally.
...
// Linear time O(n) -- basic operation performed on each character in the string one by one.  If the string size increases, the time needed will increase proportionally.
...
// Linear time O(n) -- the larger the n, the more time it will take--function is called for each lower number
...
function split(str, sep) {
  ...
  //REMEMBER THAT SPLIT IS THE NAME OF THE FUNCTION
  //all these are valid syntax
  //return (str.slice(0,idx) + (split(str.slice(idx + sep.length), sep)))
  //return str.slice(0,idx).concat(split(str.slice(idx + sep.length), sep))
}

// Linear time O(n) -- the larger the string and/or the more times the separator is present, the longer it will take
...
//Logarithmic time O(log(n)) -- takes a little more time as the number is larger
...
// Linear time O(n) -- as the number gets larger, the time requried increases proportionally
